...
from src.evaluating.query_processor import IQueryProcessor
...
from src.evaluating.scorer.similarity_scorer import SimilarityScorer
...
class FactScoreProcessor(IQueryProcessor):
    def __init__(self, faiss_manager, scorer: IScorer, ):
        ...
        self.scorer = scorer
        with open("data/out/subclaims_schema.json", 'r', encoding='utf-8') as schemafile:
    ...
    def annotate_subclaim(self, subclaim_file):
        with open(subclaim_file, 'r', encoding='utf-8') as jsonfile:
            ...
            for entry in subclaims_data:
                validate(instance=entry, schema=self.subclaim_schema)
                retrieved_docs = self.scorer.faiss_manager.search_faiss_index(entry["query"], 10, 0.3)
                doc_contents = []
                ...
                context = "\n".join(doc_contents)
                for subclaim in entry["subclaims"]:
                    annotation = self.verifier.annotate(entry["query"], entry["gld_ans"], context, subclaim["subclaim"])
...
if __name__ == "__main__":
    document_file = FileManager("data/processed/FactScore/title_text_map_factscore.txt")
    ...
    processor.get_subclaims("data/processed/FactScore/fact_score_queries.json", "data/out/FactScore/factscore_subclaims.json")
    processor.score_subclaim("data/out/FactScore/factscore_subclaims.json")
    processor.annotate_subclaim("data/out/FactScore/factscore_subclaims.json")