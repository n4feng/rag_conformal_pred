...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, required=True, help="Name of the dataset", choices=["fact_score", "hotpot_qa", "pop_qa", "medlfqa"])
    ...
    elif args.dataset == "medlfqa":
        raw_data_dir = "data/raw/MedLFQA"
        processed_data_dir = "data/processed/MedLFQA"
    
    # Load data
    if not os.path.exists(f"{raw_data_dir}/raw_{args.dataset}.json"):
        
        ouptut_path=f"{raw_data_dir}" if args.dataset == "medlfqa" else f"{raw_data_dir}/raw_{args.dataset}.json"
        
        data_loader = DataLoader(args.dataset)
        data_loader.load_qa_data(ouptut_path=ouptut_path)
    ...
    if not os.path.isfile(f"data/raw/WikiDB/{args.wiki_db_dir}"):
        ...
        input_file = f"{raw_data_dir}/raw_{args.dataset}.json" if args.dataset != "medlfqa" else f"data/raw/MedLFQA"
        
        query_processor = QueryProcessor(db_path=f"data/raw/WikiDB/{args.wiki_db_dir}")
        # create queries data
        query_processor.get_queries(dataset=args.dataset, input_file=input_file, output_file=f"{processed_data_dir}/{args.dataset}_queries.json")
